class Solution {
    ...
    private void dfs(int nx, int ny, char[][] board, boolean[][] vis) {
        if(nx<0 || ny<0 || nx>=board.length || ny>=board[0].length || vis[nx][ny] || board[nx][ny] == 'X') return;
        vis[nx][ny] = true;
        dfs(nx+1, ny, board, vis);
        dfs(nx-1, ny, board, vis);
        dfs(nx, ny+1, board, vis);
        dfs(nx, ny-1, board, vis);
    }
